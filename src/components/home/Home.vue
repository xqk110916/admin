  ...
  <div class="app">
    <Row class="top">
      <Col :xs="13" :sm="7" style="font-size:14px; color: rgb(0, 122, 255);">
        <Icon type="md-menu" size="24" @click="drawerBtn = true"></Icon>
    ...
    <Row class="claecla">
      <Row class="code" v-for="item in newData" v-bind:key="item.code">
        <span>{{ changeVarietyName(item) }}</span>
        <span @click="goTpage(item.market, item.code, changeDate(expiryDate))">{{ item.code }}</span>
        <span :class="{ red: item.call , green: item.put }"
          @click="goTpage(item.market, item.code, expiryDate)">{{ item.price }}</span>
        <span :class="{ red: item.call , green: item.put }" @click="detailedPrice(item.code)">{{ item.buy }}</span>
        <span :class="{ red: item.call , green: item.put }" @click="detailedPrice(item.code)">{{ item.sale }}</span>
      </Row>
    </Row>
    ...
    <Modal :title="Title" v-model="drawerModal" :footer-hide="true" class-name="vertical-center-modal">
      ...
      <Row>
        ...
        <p>期权类型</p>
        <p class="sele-p">
          <Select v-model="direction" @on-change="priceChange">
        ...
        <p>标的价格</p>
        <p class="sele-p">
          <InputNumber v-model="detailedData.thePrice" placeholder="0" @on-change="priceChange"></InputNumber>
        </p>
        </Col>
      </Row>
      <Row>
        ...
        <p>执行价</p>
        <p class="sele-p">
          <InputNumber v-model="detailedData.strikePrice" placeholder="0" @on-change="priceChange"></InputNumber>
        </p>
        </Col>
        <Col span="12" class="modal-row">
        <p>成交数量</p>
  ...
  export default {
      ...
      bus.$on('tDefinedEvent', function (msg) {
        ...
        for (var i = 0; i < that.$store.state.codeListStore.length; i++) {
          if (msg.code === that.$store.state.codeListStore[i].code) {
            ...
            that.detailedData.marginRate = that.add(that.$store.state.instrumentStore[i].margin_rate[1].long_margin_ratio_by_money, 0.01)
          }
        }
        // that.priceChange()
        that.detailedPrice()
    ...
    methods: {
      ...
      detailedPrice: function (c) {
        ...
        this.direction = 'C'
        console.log(this.detailedData.strikePrice)
        for (var i = 0; i < this.newData.length; i++) {
          if (c === this.newData[i].code) {
            ...
            this.detailedData.marginRate = this.add(this.$store.state.instrumentStore[i].margin_rate[1].long_margin_ratio_by_money, 0.01)
            this.Title = this.newData[i].code + 'C' + this.changeDate(this.expiryDate) + 'M' + this.detailedData.strikePrice
        ...
        this.detailedData.margin = Math.max(this.sub(this.detailedData.tradingMargin, (this.mul(0.5,VirtualValue))),(this.mul(0.5,VirtualValue)));
      ...
      priceChange: function () {
        ...
        that.websock.onmessage = function (evt) {
          ...
          that.detailedData.margin = Math.max(that.sub(that.detailedData.tradingMargin, (that.mul(0.5,VirtualValue))),(that.mul(0.5,VirtualValue)));
        }
      },
