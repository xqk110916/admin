<template>
  <div class="t-top">
    ...
    <Row class="code" v-for="item in records" v-bind:key="item.id">
      <span style="color: #16d397" @click="homeBus('C', item.price, item.BuyCall)">{{ item.BuyCall }}</span>
      <span style="color: #e75563" @click="homeBus('C', item.price, item.BuyCall)">{{ item.SaleCall }}</span>
      <span>{{ item.price }}</span>
      <span style="color: #16d397" @click="homeBus('P', item.price, item.BuyCall)">{{ item.BuyPut }}</span>
      <span style="color: #e75563" @click="homeBus('P', item.price, item.BuyCall)">{{ item.SalePut }}</span>
    </Row>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      options3: {
        disabledDate: date => {
          return date.getDay() === 0 || date.getDay() === 6 || this.$store.state.restDayStore.indexOf(Number(this.changeDate(date))) !== -1 || this.changeDate(date) < this.minDays || this.changeDate(date) > this.deadline
        }
      }
    }
  },
  created: function () {
    ...
    let dT = this.DateDiff(this.nowDate, expiryDateWebs)
    
    let deduction = this.Deduction(this.nowDate, expiryDateWebs, this.$store.state.festivalListStore)
    let dBuy = ((dT - deduction[0]) / 365)
    let dSale = ((dT - deduction[1]) / 365)
    let that = this
    let heartCheck = {
      timeout: 50000,
      timeoutObj: null,
    ...
    that.websock.onmessage = function (evt) {
      ...
      for (let i = 0; i < that.records.length; i++) {  
        ...
        if (that.records[i].bid_volatility !== 0 && that.records[i].price !== 0) {
          that.records[i].SaleCall = that.CalcCall(that.price, that.records[i].bid_volatility, that.interestRate, that.records[i].price, dBuy)
          that.records[i].SaleCall = (that.records[i].SaleCall).toFixed(2)
          that.records[i].BuyCall = that.CalcCall(that.price, that.records[i].ask_volatility, that.interestRate, that.records[i].price, dSale)
          that.records[i].BuyCall = (that.records[i].BuyCall).toFixed(2)
          that.records[i].SalePut = that.CalcPut(that.price, that.records[i].bid_volatility, that.interestRate, that.records[i].price, dBuy)
          that.records[i].SalePut = (that.records[i].SalePut).toFixed(2)
          that.records[i].BuyPut = that.CalcPut(that.price, that.records[i].ask_volatility, that.interestRate, that.records[i].price, dSale)
...
.code span:nth-child(3) {background-color: rgb(245, 245, 245) !important}
.code:nth-child(10) span:nth-child(3) {color: black !important}
</style>
