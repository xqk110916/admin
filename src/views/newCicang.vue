  ...
  <div id="chicang" class="Transactions_contrnt">
    <HeadBack title="查看前收盘持仓">
      <el-button @click="daochu" class="daochu" size="mini" type="primary" :disabled="daochudisabled" slot="btn"
        style="position:absolute;right:5px;top:6px;">导出实时持仓</el-button>
    </HeadBack>
        ...
        <template slot-scope="props">
          <el-form label-position="left" inline class="demo-table-expand cicang"
            ...
            <el-form-item label="报价时间：">
              <span>{{ item.quoteTime }}</span>
            </el-form-item>
            <el-form-item label="标的价格：">
              <span>{{ item.currentUnderlyingPrice }}</span>
            </el-form-item>
          </el-form>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      //持仓分页
      PositionChange(val) {
        axios.post('trade/queryCurrentTimeContract.do', this.qs.stringify({
      ...
      daochu() {
        window.open(`${g.baseURL}/client/inner/excel/exportRepairCurrentTimeClientContract.do?webClientBindId=${this.userID}`)
      },
      // 分页器的事件处理函数
      current(page) {
        this.page = page
