<template>
  <div id="edit_phone">
    <HeadBack title="修改手机号码"></HeadBack>
    <el-form :model="ruleForm2" status-icon :rules="rules2" ref="ruleForm2" label-width="100px" class="demo-ruleForm" label-position="right">
      <el-form-item label="原手机号" prop="oldPhone">
        <el-input type="text" v-model="ruleForm2.oldPhone" auto-complete="off" placeholder="请输入原绑定手机号"></el-input>
      </el-form-item>
      ...
      <el-form-item label="新手机号" prop="phone">
        <el-input v-model.number="ruleForm2.phone" placeholder="请输入绑定手机号"></el-input> 
...
<script>
  ...
  export default {
    name: "pushPhone",
    data() {
      var valoldPhone = (rule, value, callback) => {
        if (!value) {
    ...
    methods: {
      getyzm(){
        ...
        axios.post('client/sendValCode.do',this.qs.stringify({
          pe: this.ruleForm2.phone,
			    verificationCodeType: 'UPDATE_LOGIN_PHONENO_CODE'
        })).then(result => {
      ...
      submitForm(formName) {
        ...
        if(this.can){
          axios.post('client/updateClientBaseInfoPhoneno.do',this.qs.stringify({
            oldPhoneno: this.ruleForm2.oldPhone,
            loginPassword: this.ruleForm2.pass,
            newPhoneno: this.ruleForm2.phone,
            valcode: this.ruleForm2.yzm.toString().replace(/^\s*|\s*$/g,"")
          })).then(res => {
            if(res) {
              this.ruleForm2.oldPhone = ''
              this.ruleForm2.pass = ''
